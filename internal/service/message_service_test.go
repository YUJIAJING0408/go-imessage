...
func TestSendMessage_ValidRequest_CreatesMessageAndAttempt(t *testing.T) {
	...
	msg, att, err := svc.SendMessage(req)
	if err != nil {
...
func TestSendMessage_IdempotentWithClientMsgID(t *testing.T) {
	...
	first, _, _ := svc.SendMessage(req)
	second, _, _ := svc.SendMessage(req)
...
func TestSendMessage_InvalidRequest_Rejected(t *testing.T) {
	svc := newSvc()
	_, _, err := svc.SendMessage(SendMessageRequest{Content: ""})
	if err == nil {
		t.Error("空内容应返回错误")
	}
}
...
func TestRetryMessage_StartsNewAttemptAndUpdatesSenderPreview(t *testing.T) {
	svc := newSvc()
	msg, att, _ := svc.SendMessage(SendMessageRequest{
		SenderID:       1,
		ReceiverID:     2,
		ConversationID: 10,
		Content:        "重试测试",
	...
	// 先将第一次发送标记为失败
	_, err := svc.CompleteAttempt(CompleteAttemptRequest{AttemptID: att.ID, Success: false, ErrorCode: "timeout"})
	if err != nil {
		t.Fatal(err)
	}
	// 执行重试
	retriedMsg, retryAtt, err := svc.RetryMessage(msg.ID)
	if err != nil {
...
func TestCompleteAttempt_UpdatesStatusAndSummary(t *testing.T) {
	...
	_, err := svc.CompleteAttempt(CompleteAttemptRequest{AttemptID: att.ID, Success: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteMessage_SyncsToBothUsers(t *testing.T) {
	svc := newSvc()
	msg, _, _ := svc.SendMessage(SendMessageRequest{
		SenderID:       1,
		ReceiverID:     2,
		ConversationID: 10,
		Content:        "删除测试",
	...
	// 发送方同步应包含删除事件
	senderEvents, _, _ := svc.Sync(SyncRequest{UserID: 1})
	found := false
	...
	// 接收方同步也应包含删除事件
	receiverEvents, _, _ := svc.Sync(SyncRequest{UserID: 2})
	found = false
...
func TestSync_ReturnsEventsBasedOnCursor(t *testing.T) {
	svc := newSvc()
	svc.SendMessage(SendMessageRequest{
		SenderID:       1,
		ReceiverID:     2,
		ConversationID: 10,
		Content:        "同步1",
	...
	events, next, err := svc.Sync(SyncRequest{UserID: 1, DeviceID: "d1"})
	if err != nil {
		t.Fatal(err)
	}
	if len(events) == 0 {
	...
	// 使用返回的游标再次同步，应无新事件
	events2, _, _ := svc.Sync(SyncRequest{UserID: 1, DeviceID: "d1", Cursor: next})
	if len(events2) != 0 {
